...
class FluentCommunityConnect extends BaseIntegrationManager
{
    ...
    public function getSettingsFields($settings, $args = [])
    {
        ...
        $fields = [
            ...
            'space_ids'              => [
                'key'         => 'space_ids',
                'label'       => __('Add to Spaces', 'webmakerr-cart'),
                'placeholder' => __('Select FluentCommunity Spaces', 'webmakerr-cart'),
                'inline_tip'  => __('Select the FluentCommunity Spaces you would like to add.', 'webmakerr-cart'),
                'component'   => 'select',
                'is_multiple' => true,
                'required'    => false,
            ...
            'course_ids'             => [
                'key'          => 'course_ids',
                'require_list' => false,
                'label'        => __('Add to Courses', 'webmakerr-cart'),
                'placeholder'  => __('Select Courses', 'webmakerr-cart'),
                'component'    => 'select',
                'is_multiple'  => true,
                'options'      => $formattedCourses,
                'inline_tip'   => __('Select the courses you would like to enroll the customer to', 'webmakerr-cart')
            ],
            'remove_space_ids'       => [
                'key'         => 'remove_space_ids',
                'label'       => __('Remove From Spaces', 'webmakerr-cart'),
                'placeholder' => __('Select Spaces', 'webmakerr-cart'),
                'inline_tip'  => __('Select the Spaces you would like to remove from your spaces.', 'webmakerr-cart'),
                'component'   => 'select',
                'is_multiple' => true,
                'required'    => false,
                'options'     => $formattedSpaces
            ...
            'remove_course_ids'      => [
                ...
                'require_list' => false,
                'label'        => __('Remove From Courses', 'webmakerr-cart'),
                'placeholder'  => __('Select Courses', 'webmakerr-cart'),
                'component'    => 'select',
                'is_multiple'  => true,
                'options'      => $formattedCourses,
                'inline_tip'   => __('Select the courses you would like to remove from the customer', 'webmakerr-cart')
            ],
            'mark_as_verified'       => [
                'key'            => 'mark_as_verified',
                'component'      => 'yes-no-checkbox',
                'checkbox_label' => __('Mark the community profile as verified', 'webmakerr-cart'),
                'inline_tip'     => __('If you enable this, the user will be marked as verified in FluentCommunity', 'webmakerr-cart')
            ],
            'watch_on_access_revoke' => [
                'key'            => 'watch_on_access_revoke',
                'component'      => 'yes-no-checkbox',
                'checkbox_label' => __('Remove from selected Courses/Spaces on Refund or Subscription Access Expiration ', 'webmakerr-cart'),
                'inline_tip'     => __('If you enable this, on refund or subscription validity expiration, the selected spaces and courses will be removed from the customer.', 'webmakerr-cart')
            ]
        ];
        ...
        return [
            ...
            'integration_title'   => __('FluentCommunity', 'webmakerr-cart')
        ];
    }
    ...
    public function processAction($order, $eventData)
    {
        ...
        if ($isRevokedHook) {
            ...
            if ($removedIds) {
                $order->addLog(
                    __('FluentCommunity Access Removed', 'webmakerr-cart'),
                    sprintf(
                        /* translators: %s is the space ids */
                        __('User has been removed from spaces and courses: %s', 'webmakerr-cart'), implode(', ', $removedIds)),
                    'info',
                    'FluentCommunity'
                );
            }
        ...
        if (!$userId) {
            ...
            if (is_wp_error($userId)) {
                $order->addLog(
                    __('User creation failed from FluentCommunity Integration', 'webmakerr-cart'),
        ...
        $order->addLog(
            __('FluentCommunity Integration Success', 'webmakerr-cart'),
            sprintf(
                /* translators: 1: space ids, 2: course ids */
                __('User has been added to spaces: %1$s and courses: %2$s', 'webmakerr-cart'), implode(', ', $spaceIds), implode(', ', $courseIds)),
            'info',
            'FluentCommunity'
        );
    }
