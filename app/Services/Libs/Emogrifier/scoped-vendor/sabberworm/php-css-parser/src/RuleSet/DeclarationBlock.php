...
namespace FluentEmogrifier\Vendor\Sabberworm\CSS\RuleSet;

use FluentEmogrifier\Vendor\Sabberworm\CSS\CSSList\CSSList;
use FluentEmogrifier\Vendor\Sabberworm\CSS\CSSList\KeyFrame;
use FluentEmogrifier\Vendor\Sabberworm\CSS\OutputFormat;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Parsing\OutputException;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Parsing\ParserState;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Parsing\UnexpectedEOFException;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\KeyframeSelector;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\Selector;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Rule\Rule;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\Color;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\RuleValueList;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\Size;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\URL;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\Value;
/**
...
class DeclarationBlock extends RuleSet
{
            ...
            if (!$mSelector instanceof Selector) {
                if ($oList === null || !$oList instanceof KeyFrame) {
                    ...
                    if (!KeyframeSelector::isValid($mSelector)) {
                        throw new UnexpectedTokenException(
                            sprintf(
                                /* translators: %s is the regex pattern being validated */
                                "Selector did not match '%s'.",
                                esc_html( KeyframeSelector::SELECTOR_VALIDATION_RX )
                            ),
                            esc_html($mSelector),
                            "custom"
                        );
    ...
    public function render(OutputFormat $oOutputFormat)
    {
        ...
        if (\count($this->aSelectors) === 0) {
            ...
            throw new OutputException("Attempt to print declaration block with missing selector", esc_html($this->iLineNo));
        }
