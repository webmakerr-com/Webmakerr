...
class FileSystemHelper extends \Box\Spout\Common\Helper\FileSystemHelper implements FileSystemWithRootFolderHelperInterface
{
    ...
    {
        $manifestXmlFileContents = '<?xml version="1.0" encoding="UTF-8"?>
...
';
    ...
    protected function createMetaFile()
    {
        ...
        $metaXmlFileContents = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
';
    ...
    public function createContentFile($worksheetManager, $styleManager, $worksheets)
    {
        ...
        // ---------- Build the initial XML header ----------
        $contentXml  = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' . "\n";
        $contentXml .= '<office:document-content office:version="1.2" ';
        $contentXml .= 'xmlns:dc="http://purl.org/dc/elements/1.1/" ';
        $contentXml .= 'xmlns:calcext="urn:org:documentfoundation:names:experimental:calc:xmlns:calcext:1.0" ';
        $contentXml .= 'xmlns:draw="urn:oasis:names:tc:opendocument:xmlns:drawing:1.0" ';
        $contentXml .= 'xmlns:fo="urn:oasis:names:tc:opendocument:xmlns:xsl-fo-compatible:1.0" ';
        $contentXml .= 'xmlns:msoxl="http://schemas.microsoft.com/office/excel/formula" ';
        $contentXml .= 'xmlns:number="urn:oasis:names:tc:opendocument:xmlns:datastyle:1.0" ';
        $contentXml .= 'xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" ';
        $contentXml .= 'xmlns:style="urn:oasis:names:tc:opendocument:xmlns:style:1.0" ';
        $contentXml .= 'xmlns:svg="urn:oasis:names:tc:opendocument:xmlns:svg-compatible:1.0" ';
        $contentXml .= 'xmlns:table="urn:oasis:names:tc:opendocument:xmlns:table:1.0" ';
        $contentXml .= 'xmlns:text="urn:oasis:names:tc:opendocument:xmlns:text:1.0" ';
        $contentXml .= 'xmlns:xlink="http://www.w3.org/1999/xlink">';
        $contentXml .= "\n";

        $contentXml .= $styleManager->getContentXmlFontFaceSectionContent();
        $contentXml .= $styleManager->getContentXmlAutomaticStylesSectionContent($worksheets);
        $contentXml .= '<office:body><office:spreadsheet>';

        // ---------- Write the initial part ----------
        $contentXmlFilePath = trailingslashit($this->rootFolder) . self::CONTENT_XML_FILE_NAME;
        $wp_filesystem->put_contents($contentXmlFilePath, $contentXml, FS_CHMOD_FILE);

        // ---------- Append all worksheets ----------
        foreach ( $worksheets as $worksheet ) {
            $sheetContent  = $worksheetManager->getTableElementStartAsString($worksheet);

            // append the worksheet content file
            $worksheetFilePath = $worksheet->getFilePath();
            if ( $wp_filesystem->exists( $worksheetFilePath ) ) {
                $sheetContent .= $wp_filesystem->get_contents( $worksheetFilePath );
            }

            $sheetContent .= '</table:table>';

            // append to file
            $existing = $wp_filesystem->get_contents( $contentXmlFilePath );
        ...
        // ---------- Close the document ----------
        $footer = '</office:spreadsheet></office:body></office:document-content>';
        $existing = $wp_filesystem->get_contents( $contentXmlFilePath );
        $wp_filesystem->put_contents(
            $contentXmlFilePath,
            $existing . $footer,
