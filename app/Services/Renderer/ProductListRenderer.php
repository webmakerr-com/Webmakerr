...
class ProductListRenderer
{
    ...
    public function render()
    {
        ...
        ?>
        <section class="fct-product-list-container <?php echo esc_attr($this->wrapperClass); ?>" aria-label="<?php echo esc_attr($this->listTitle ?: __('Product List', 'webmakerr-cart')); ?>">
            ...
            <?php if ($isSimilarProductList): ?>
                <div class="fct-product-list-slider-wrapper <?php echo $enableSlider ? 'has-slider' : ''; ?>" data-fct-similar-slider-wrapper>
                    <?php if ($enableSlider): ?>
                        <button type="button" class="fct-similar-nav-button prev" data-fct-similar-slider-prev aria-label="<?php esc_attr_e('View previous related products', 'webmakerr-cart'); ?>">
                    ...
                    <?php if ($enableSlider): ?>
                        <button type="button" class="fct-similar-nav-button next" data-fct-similar-slider-next aria-label="<?php esc_attr_e('View more related products', 'webmakerr-cart'); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
