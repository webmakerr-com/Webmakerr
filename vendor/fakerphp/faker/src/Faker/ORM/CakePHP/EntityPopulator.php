...
namespace FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\Faker\ORM\CakePHP;

use FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\Cake\ORM\TableRegistry;
class EntityPopulator
{
    ...
    public function guessColumnFormatters($populator)
    {
        ...
        $guessers = $populator->getGuessers() + ['ColumnTypeGuesser' => new \FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\Faker\ORM\CakePHP\ColumnTypeGuesser($populator->getGenerator())];
        ...
        foreach ($belongsTo as $assoc) {
            $modifiers['belongsTo' . $assoc->name()] = function ($data, $insertedEntities) use ($assoc) {
                ...
                if (!empty($insertedEntities[$foreignModel])) {
                    ...
                    $foreignKeys = $table->find('all')->select(['id'])->map(static function ($row) {
                        return $row->id;
                    })->toArray();
                }
    ...
    protected function getTable($class)
    {
        ...
        return \FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\FluentCart\Cake\ORM\TableRegistry::get($class, $options);
    }
}