 ...
 */
for (let i = 0; i <= 9; ++i) {
    // This needs to use a string cause otherwise since 0 is falsy
...
function _characterFromEvent(e) {
    ...
    if (e.type === 'keypress') {
        ...
        // that we want the character to be lowercase.
        if (!e.shiftKey) {
...
function Mousetrap(targetElement = document) {
    const self = this;
    ...
    function _resetSequences(doNotReset = {}) {
        ...
        for (const key in _sequenceLevels) {
            if (doNotReset[key]) {
    ...
    function _getMatches(character, modifiers, e, sequenceName, combination, level) {
        ...
        for (let i = 0; i < self._callbacks[character].length; ++i) {
            const callback = self._callbacks[character][i];
    ...
    self._handleKey = function (character, modifiers, e) {
        ...
        // being a modifier then we should reset all sequences that were not matched
     ...
     *
     * @param {InterpreterRequestEvent} e
     * @returns void
     */
    function _handleKeyEvent(e) {
        ...
        self._handleKey(character, _eventModifiers(e), e);
    }
...
Mousetrap.prototype.unbind = function (keys, action) {
    return this.bind(keys, function () {
    }, action);
};
...
/**
 * resets the library back to its initial state
 *
 * @returns {Mousetrap}
 */
...
Mousetrap.prototype.stopCallback = function (e, element) {
    ...
    if (_belongsTo(element, this.target)) {
        return false;
    }
 ...
 */
Mousetrap.prototype.handleKey = function (...args) {
...
/**
 * Initialize a global Mousetrap instance for convenience
 *
 * @returns {Mousetrap}
 */
Mousetrap.init = function () {
    return new Mousetrap(document);
};

function useKeyboardShortcuts() {
    const mt =  Mousetrap.init();
    mt.stopCallback = function (e, element) {
        // Allow all shortcuts even inside input/textarea
        return false;
    };
    return mt;
}

// Export the Mousetrap constructor
