...
import App from "@/elementor/DirectCheckout/App.vue";
...
jQuery(document).ready(function () {
    elementor.hooks.addAction('panel/open_editor/widget/fluent_cart_direct_checkout', function (panel, model, view) {
...
jQuery(window).on('elementor/frontend/init', function () {
    ...
    //     if (widgetType === 'fluent_cart_direct_checkout') {
    //     }
    // });

    elementorFrontend.hooks.addAction('frontend/element_ready/fluent_cart_direct_checkout.default', function ($scope) {
        const container = jQuery($scope[0]).find('[data-fluent-cart-direct-checkout-app]')[0];
        mountApp(container);
    });
});
