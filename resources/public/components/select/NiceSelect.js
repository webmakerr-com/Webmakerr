    ...
    constructor(element, options = {}) {
        ...
        if (element.tagName === 'SELECT') {
            ...
            selectOptions.forEach(op => {
                ...
                if (op.value || op.textContent.trim()) {
                    parsedOptions.push({
                        value: op.value || op.textContent.trim(),
                        text: op.textContent.trim()
                    });
                }
            });
        }
...
`;
        ...
        // Add the event listener after DOM insertion
        // TODO: we will refactor this later
        // this.dropdown.addEventListener('focus', (e) => this.focus(e.target));
        // this.dropdown.querySelector(".fct-nice-select-search").addEventListener("focusout", (e) => {
    ...
    */
        ...
        if (this.multiple) {
            ...
            if (
                ...
                this.selectedOptions.forEach((item, index, array) => {
                    ...
                    selectedHtml += `<span class="current">${text || this.placeholder}</span>`;
                });
            ...
            // Add/remove "empty-value" class depending on value
            if (
                this.selectedOptions.length > 0 &&
                (!this.selectedOptions[0].data.value || this.selectedOptions[0].data.value === "")
            ) {
                currentEl.classList.add("empty-value");
            } else {
                currentEl.classList.remove("empty-value");
            }
        }
    }
    ...
    setSelected(value) {
        ...
        for (let i = 0; i < this.options.length; i++) {
            ...
            if (this.options[i].data.value === value) {
    ...
    focus(target = '') {
        ...
        if (hasClass(this.dropdown, "open")) {
            const search = this.dropdown.querySelector(".fct-nice-select-search");
    ...
    _bindSearchEvent() {
        const searchBox = this.dropdown.querySelector(".fct-nice-select-search");
        if (searchBox) {
    ...
    */
    #syncSelectValue() {
        ...
        if (this.onChange && typeof this.onChange === 'function') {
            this.onChange(select.value, select);
    ...
    */
    ...
    setOptions(newOptions, nameAccessor = null, valueAccessor = null) {
        ...
        // Clear current selected options that don't exist in new options
        if (this.selectedOptions.length > 0) {
            const newValues = this.data.map(opt => opt.value);
            const validSelectedOptions = this.selectedOptions.filter(selected =>
                newValues.includes(selected.data.value)
            );

            // If no valid selections remain, clear all
            if (validSelectedOptions.length === 0) {
                this.clear();
            } else {
                this.selectedOptions = validSelectedOptions;
            }
        }

        // Update the original select element with new options
    ...
    #updateSelectElement() {
        ...
        this.data.forEach(optionData => {
            ...
            if (optionData.extra) {
                option.dataset.extra = optionData.extra;
            }
